# Power BI — Retail Analytics Layer (Phase 3)
...
| File | Description |
|------|-------------|
| `Retail_Analysis.pbit` | **Power BI template** — open in Power BI Desktop, point it at `powerbi/dataset/`, refresh. Contains the complete star-schema model, 4 relationships, 29 DAX measures and all data-load queries (see "What's in the .pbit"). |
...
| `dataset/` | Power BI-ready star-schema CSVs: FactSales (527,390), DimDate (730), DimCustomer (4,339, RFM), DimProduct (3,947), DimCountry (38). |
| `scripts/export_pbi_dataset.py` | Regenerates `dataset/` straight from PostgreSQL. |
| `scripts/validate_pbi.py` | Reconciles `dataset/` against PostgreSQL/SQL figures (24 checks, all PASS). |
| `data_model.md` | Star-schema design, columns, relationships, region groupings. |
| `dax_measures.md` | All 29 measures with business purpose and formatting. |
| `pages.md` | Full page-by-page spec for the 5 report pages + drill-through. |
| `validation.md` | Validation report: benchmarks, RFM reconciliation, limitations. |
| `previews/` | Portfolio-ready previews of all 5 pages (fixed 1280×800 canvas, **zero overflow**, QA-verified) rendered with real validated data — HTML + PNG screenshots + the `generate_preview.py` script. |
...
The model is then ready: 6 tables, 4 relationships, 29 measures in a dedicated
...
- FactSales, DimDate, DimCustomer (validated RFM quartiles + 7 segments),
  DimProduct, DimCountry, Measures.
- 4 one-to-many relationships (DimDate → FactSales, DimCustomer → FactSales,
  DimProduct → FactSales, DimCountry → FactSales).
- 29 DAX measures (Core KPIs, Value, Customer, Time, Ranking) with currency /
  percent / integer formatting and display folders.
- M data-load queries for all 5 tables via the `DatasetFolder` parameter.
...
`previews/` contains static, no-JS HTML renderings of all 5 pages **using the
real validated numbers** (open the `.html` files in any browser), plus PNG
...
the canvas (all 5 currently **QA_OK**). They follow the `pages.md` design
